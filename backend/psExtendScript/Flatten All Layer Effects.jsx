...
/*
@@@BUILDINFO@@@ Flatten All Layer Effects.jsx 1.0.0.3
*/
...
function main(){
	...
	// FORK: Second pass — handle clip-base layers.
	// flattenLayerStyles() inserts an empty layer BELOW the target and merges,
	// which silently fails for clip bases because the empty layer lands outside
	// the clip group. Instead, rasterize clip-base effects in-place (no new layer,
	// no merge, clip relationships are preserved).
	try {
		flattenClipBaseStyles(doc);
	} catch(e) {
		; // do nothing
	}
...
///////////////////////////////////////////////////////////////////////////////
// FORK: Functions for handling clip-base layers
///////////////////////////////////////////////////////////////////////////////

///////////////////////////////////////////////////////////////////////////////
// Function: flattenClipBaseStyles
// Usage: Walk parent.layers in panel order; for each clip-base ArtLayer that
//        has a layer style, rasterize the style in-place. Recurse into LayerSets.
// Input: parent — document or LayerSet
// Return: <none>
///////////////////////////////////////////////////////////////////////////////
function flattenClipBaseStyles(parent) {
	var arr = parent.layers;
	// layers[0] = topmost in panel, layers[N-1] = bottommost.
	// A clip base: arr[i] is NOT grouped, but arr[i-1] IS grouped.
	for (var i = 1; i < arr.length; i++) {
		try {
			if (!isGroupedLayer(arr[i]) && isGroupedLayer(arr[i - 1])) {
				// arr[i] is a clip base
				if (arr[i].typename === "ArtLayer") {
					doc.activeLayer = arr[i];
					if (hasLayerStyle()) {
						rasterizeLayerStyleInPlace();
					}
				}
				// LayerSet clip bases: merge the group first, then rasterize.
				// Handled conservatively — skip to avoid unintended side-effects.
			}
		} catch(e) {
			; // do nothing
		}
		// Recurse into sub-groups regardless of whether arr[i] is a clip base
		try {
			if (arr[i].typename === "LayerSet") {
				flattenClipBaseStyles(arr[i]);
			}
		} catch(e) {
			; // do nothing
		}
...
///////////////////////////////////////////////////////////////////////////////
// Function: isGroupedLayer
// Usage: Returns true if the layer is "grouped" (clipped to the layer below).
//        Uses DOM layer.grouped property; falls back to ActionManager.
// Input: layer — ArtLayer or LayerSet
// Return: Boolean
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// Function: rasterizeLayerStyleInPlace
// Usage: Rasterize the layer style of the current activeLayer in-place,
//        without creating helper layers. Equivalent to
//        Layer > Rasterize > Layer Style.
// Input: <none> — operates on doc.activeLayer
// Return: <none>
///////////////////////////////////////////////////////////////////////////////
function rasterizeLayerStyleInPlace() {
	try {
