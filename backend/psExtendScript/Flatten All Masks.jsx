...
function hasVectorMask() {
    ...
    try {
        var ref = new ActionReference();
        var hasVectorMask = app.stringIDToTypeID("hasVectorMask");
        ref.putProperty( app.charIDToTypeID( 'Prpr' ), hasVectorMask);
        ref.putEnumerated( app.charIDToTypeID( 'Lyr ' ), app.charIDToTypeID( 'Ordn' ), app.charIDToTypeID( 'Trgt' ) );
        var desc = executeActionGet( ref );
        if ( desc.hasKey( hasVectorMask ) && desc.getBoolean( hasVectorMask )) {
            hasVectorMask = true;
        }
    }catch(e) {
        hasVectorMask = false;
