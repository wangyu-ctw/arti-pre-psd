...
def _build_process_applescript(input_file_name: str) -> str:
    """生成处理流程的 AppleScript（PSD 已经被 `open -a` 推给 PS 了，这里只做后续）：
       ...
       等 PS 把目标文件加载到 front document → 跑 4 个 .jsx → 关闭原文档
    ...
    为什么这里**不**让 AppleScript 自己 `open` 文件：
      ...
      - 所以让 Python subprocess 直接 `open -a PS_PATH FILE`，把活儿交给
    ...
    steps = [
        ("1. Delete All Empty Layers", "jsxDeleteText"),
        ("2. Flatten All Layer Effects", "jsxFxText"),
        ("3. Flatten Groups With Effects", "jsxFlattenGroupsText"),
        ("4. Flatten All Masks", "jsxMasksText"),
        ("5. Flatten Clipping Masks", "jsxClippingText"),
        ("5b. Delete Non-Normal / Adjustment Clip Layers", "jsxDelProblematicClipText"),
        ("5c. Flatten Clipping Masks (再跑)", "jsxClippingText"),
        ("6. Trim Layers To Canvas", "jsxTrimText"),
        ("1b. Delete All Empty Layers (再跑)", "jsxDeleteText"),
    ...
    return (
        # 先在 osascript 进程内把 7 个 jsx 内容读成 UTF-8 字符串（read 是 StandardAdditions）
...
def process_psd(input_path: str, ps_path: str) -> dict[str, Any]:
    """主流程（仅 macOS）：
      ...
      1. 用 `open -a PS_PATH FILE` 让 LaunchServices 把 PSD 交给 PS
         （比 AppleScript 自己 open alias 稳得多——绕开跨进程 alias 失效的坑）
      2. 跑 osascript：等 PS 加载 → 跑 4 个 jsx → 关原文件 → return 输出路径
    """
    if not IS_MAC:
    ...
    # 检查所有脚本都在（步骤 5 + 5b 都需要）
    for script in (SCRIPT_DELETE_EMPTY, SCRIPT_FLATTEN_FX,
                   ...
                   SCRIPT_TRIM_TO_CANVAS, SCRIPT_SAVE_AS_CLEAN):
    ...
    # 2. AppleScript 等加载 + 跑 4 个 jsx + 关文档 + return 输出路径
